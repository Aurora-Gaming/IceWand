...
{
	List<IceWandAction> Actions = new();
	...
	void IceWandCmd(CommandArgs e)
	{
		...
		if (actionName == "help" || actionName == "list")
		{
			...
			e.Player.SendInfoMessage("Available actions: " + actions.ToString()[..(actions.Length - 2)]);
		}
	...
	{
		int ID = Projectile.NewProjectile(Projectile.GetNoneSource(), e.X * 16 + 8, e.Y * 16 + 8, 0, 0, 28, 250, 10);
	...
	{
		int ID = Projectile.NewProjectile(Projectile.GetNoneSource(), e.X * 16 + 8, e.Y * 16 + 8, 0, 0, 108, 250, 10);
		TSPlayer.All.SendData(PacketTypes.ProjectileNew, "", ID);
	}
	...
	{
		_ = Terraria.Item.NewItem(Projectile.GetNoneSource(), e.X * 16, e.Y * 16, 0, 0, e.Data, 1);
	}
	...
	{
		int ID = NPC.NewNPC(Projectile.GetNoneSource(), e.X * 16, e.Y * 16, e.Data);
